...
public:
  ...
  typedef Eigen::Matrix<double, StateGroup::DIM_OF_NOISE_,
                        StateGroup::DIM_OF_NOISE_>
      processNoise;
  ...
  void Process(const MeasureGroup &meas, StateGroup &state,
               pcl::PointCloud<PointType>::Ptr pcl_un_);
  V3D get_angvel();
...
private:
  ...
  void undistort_pcl(const MeasureGroup &meas, StateGroup &state_inout,
                     pcl::PointCloud<PointType> &pcl_in_out);
  void predict(StateGroup &state_inout, processNoise &Q, const V3D &gyr,
               const V3D &acc, double dt);
...
void ImuProcess::IMU_init(const MeasureGroup &meas, StateGroup &state_inout,
                          int &N) {
  ...
  // Get the gravity from common_lib.h, and use it with the unit gravity of the
  // acceleration measurement mean to find the gravity acceleration of the
  // rotation matrix type of SO2
  // note: align imu gravity to negative z axis
  bool gravity_align = true;
  if (gravity_align) {
    state_inout.rot_end = Eigen::Quaterniond::FromTwoVectors(
        -mean_acc / mean_acc.norm(), V3D(0.0, 0.0, -1.0)).toRotationMatrix();
    state_inout.grav = S2(V3D(0, 0, -G_m_s2));
  } else {
  ...
  Eigen::Matrix<double, StateGroup::DIM_OF_STATE_DOF_,
      ...
      init_P;
  init_P = Eigen::Matrix<double, StateGroup::DIM_OF_STATE_DOF_,
                         StateGroup::DIM_OF_STATE_DOF_>::Identity() *
           1e-3;
  init_P.block<3, 3>(StateGroup::rot_ext_, StateGroup::rot_ext_) =
      M3D::Identity() * 1e-5; // V3D(1e-5, 1e-5, 1e-5).asDiagonal();
  init_P.block<3, 3>(StateGroup::pos_ext_, StateGroup::pos_ext_) =
      M3D::Identity() * 1e-5;
  init_P.block<3, 3>(StateGroup::gyr_, StateGroup::gyr_) =
      M3D::Identity() * 1e-3;
  init_P.block<3, 3>(StateGroup::acc_, StateGroup::acc_) =
      M3D::Identity() * 1e-2;
  init_P.block<2, 2>(StateGroup::grav_, StateGroup::grav_) =
      Eigen::Matrix2d::Identity() * 1e-5;
  state_inout.P_ = init_P;
}

void ImuProcess::predict(StateGroup &state_inout, processNoise &Q,
                         const V3D &gyr, const V3D &acc, double dt) {
  ...
  F_x.block<3, 3>(StateGroup::vel_, StateGroup::acc_) =
      -state_inout.rot_end * dt;
  ...
  state_inout.pos_end = state_before.pos_end + state_before.vel_end * dt +
                        0.5 * acc_imu * dt * dt;
  ...
  F_x.block<2, 2>(StateGroup::grav_, StateGroup::grav_) =
      Nx * M3D::Identity() * Mx;

  state_inout.P_ =
      F_x * state_inout.P_ * F_x.transpose() + F_w * Q * F_w.transpose();
}
...
void ImuProcess::undistort_pcl(const MeasureGroup &meas,
  ...
  IMUpose.clear();
  IMUpose.push_back(set_pose6d(0.0, imu_state.pos_end, imu_state.vel_end,
                               imu_state.rot_end, acc_last, angvel_last));
  ...
  for (auto it_imu = v_imu.begin(); it_imu < (v_imu.end() - 1); it_imu++) {
    ...
    acc_last =
        imu_state.rot_end * (acc_avr - imu_state.bias_a) + imu_state.grav.vec;

    double offs_t = tail->header.stamp.toSec() - pcl_beg_time;
    IMUpose.push_back(set_pose6d(offs_t, imu_state.pos_end, imu_state.vel_end,
                                 imu_state.rot_end, acc_last, angvel_last));
  }
  ...
  for (auto it_kp = IMUpose.end() - 1; it_kp != IMUpose.begin(); it_kp--) {
    ...
    for (; it_pcl->curvature / double(1000) > head->offset_time; it_pcl--) {
       ...
       * P_compensate = R_imu_e ^ T * ((R_i * P_i + T_i) - T_e) where T_ei is
      ...
      V3D P_compensate = imu_state.rot_ex_i2l.transpose() *
                         (imu_state.rot_end.transpose() *
                              (R_i * P_i + T_i - imu_state.pos_end) -
                          imu_state.pos_ex_i2l); // not accurate!
...
V3D ImuProcess::get_angvel() { return this->angvel_last; }

void ImuProcess::Process(const MeasureGroup &meas, StateGroup &state,
                         pcl::PointCloud<PointType>::Ptr cur_pcl_un_) {
  if (meas.imu.empty()) {
    return;
  }
