...
inline bool isValid(double x) { return std::abs(x) > 1e8 ? true : false; }

PointProcess::PointProcess() {

  nh.param<std::string>("common/lid_topic", lidar_topic, "/livox/lidar");
  nh.param<std::string>("common/radar_topic", radar_topic,
                        "/radar_enhanced_pcl");
  nh.param<std::string>("common/topic_twist", topic_twist, "/eagle_data/twist");
  nh.param<std::string>("common/topic_inlier_pc2", topic_inlier_pc2,
                        "/eagle_data/inlier_pc2");
  nh.param<std::string>("common/topic_outlier_pc2", topic_outlier_pc2,
                        "/eagle_data/outlier_pc2");
  ...
  switch (lidar_type) {
  case AVIA:
    subPoints = nh.subscribe<livox_ros_driver::CustomMsg>(
        lidar_topic, 1000, &PointProcess::avia_handler, this,
        ros::TransportHints().tcpNoDelay());
    break;

  case HEASI:
    subPoints = nh.subscribe<sensor_msgs::PointCloud2>(
        lidar_topic, 1000, &PointProcess::hesai_handler, this,
        ros::TransportHints().tcpNoDelay());
    break;

  case ROBOSENSE:
    subPoints = nh.subscribe<sensor_msgs::PointCloud2>(
        lidar_topic, 1000, &PointProcess::robosense_handler, this,
        ros::TransportHints().tcpNoDelay());
    break;
  ...
  switch (radar_type) {
  case OCULII_1:
    subRadPoints = nh.subscribe<sensor_msgs::PointCloud>(
        radar_topic, 1000, &PointProcess::radar_handler1, this,
        ros::TransportHints().tcpNoDelay());
    break;
  case OCULII_2:
    subRadPoints = nh.subscribe<sensor_msgs::PointCloud2>(
        radar_topic, 1000, &PointProcess::radar_handler2, this,
        ros::TransportHints().tcpNoDelay());
    break;
  case MINDCRUISE:
    subRadPoints = nh.subscribe<sensor_msgs::PointCloud2>(
        radar_topic, 1000, &PointProcess::radar_handler3, this,
        ros::TransportHints().tcpNoDelay());
    break;
  default:
...
pcl::PointCloud<RadarPointCloudType>::ConstPtr PointProcess::distance_filter(
    const pcl::PointCloud<RadarPointCloudType>::ConstPtr &cloud) const {
  pcl::PointCloud<RadarPointCloudType>::Ptr filtered(
      new pcl::PointCloud<RadarPointCloudType>());

  filtered->reserve(cloud->size());
  std::copy_if(cloud->begin(), cloud->end(),
               std::back_inserter(filtered->points),
               [&](const RadarPointCloudType &p) {
...
pcl::PointCloud<PointT>::ConstPtr PointProcess::outlier_removal(
...
PointProcess::downsample(const pcl::PointCloud<PointT>::ConstPtr &cloud) const {
...
void PointProcess::radar_handler1(
  ...
  for (int i = 0; i < msg->points.size(); i++) {
    if (msg->channels[2].values[i] > power_threshold) {
      if (msg->points[i].x == NAN || msg->points[i].y == NAN ||
          msg->points[i].z == NAN)
        continue;
      if (msg->points[i].x == INFINITY || msg->points[i].y == INFINITY ||
          msg->points[i].z == INFINITY)
        continue;
      cv::Mat ptMat, dstMat;
      ptMat = (cv::Mat_<double>(4, 1) << msg->points[i].x, msg->points[i].y,
               msg->points[i].z, 1);
      dstMat = ptMat;
  ...
  pcl::PointCloud<RadarPointCloudType>::Ptr radarcloud_inlier(
      new pcl::PointCloud<RadarPointCloudType>);
  ...
  pcl::PointCloud<RadarPointCloudType>::ConstPtr filtered =
      distance_filter(src_cloud);
  points_pub.publish(*filtered);
}

void PointProcess::radar_handler2(
  ...
  pcl::PointCloud<RadarPointCloudType>::Ptr radarcloud_raw(
      new pcl::PointCloud<RadarPointCloudType>);
  pcl::PointCloud<EaglePointXYZIVRAB2>::Ptr radarcloud_msg(
      new pcl::PointCloud<EaglePointXYZIVRAB2>);
  ...
  for (int i = 0; i < radarcloud_msg->points.size(); i++) {
    if (radarcloud_msg->points[i].Power > power_threshold) {
      if (radarcloud_msg->points[i].x == NAN ||
          radarcloud_msg->points[i].y == NAN ||
          radarcloud_msg->points[i].z == NAN)
        continue;
      if (radarcloud_msg->points[i].x == INFINITY ||
          radarcloud_msg->points[i].y == INFINITY ||
          radarcloud_msg->points[i].z == INFINITY)
        continue;
      cv::Mat ptMat, dstMat;
      ptMat = (cv::Mat_<double>(4, 1) << radarcloud_msg->points[i].x,
               radarcloud_msg->points[i].y, radarcloud_msg->points[i].z, 1);
      dstMat = ptMat;
      radarpoint_raw.x = dstMat.at<double>(0, 0);
      radarpoint_raw.y = dstMat.at<double>(1, 0);
      radarpoint_raw.z = dstMat.at<double>(2, 0);
...
void PointProcess::radar_handler3(
    const sensor_msgs::PointCloud2::ConstPtr &msg) {
  // Implementation for MindCruise radar handler goes here
  RadarPointCloudType radarpoint_raw;
  PointT radarpoint_xyzi;
  pcl::PointCloud<RadarPointCloudType>::Ptr radarcloud_raw(
      new pcl::PointCloud<RadarPointCloudType>);
  pcl::PointCloud<TXGRadarPointType>::Ptr radarcloud_msg(
      new pcl::PointCloud<TXGRadarPointType>);
  ...
  radarcloud_xyzi->header.stamp = msg->header.stamp.toSec() * 1e6;
  for (int i = 0; i < radarcloud_msg->points.size(); i++) {
    if (radarcloud_msg->points[i].snr_db > power_threshold) {
      if (radarcloud_msg->points[i].x == NAN ||
          radarcloud_msg->points[i].y == NAN ||
          radarcloud_msg->points[i].z == NAN)
        continue;
      ...
      radarpoint_raw.y = dstMat.at<double>(1, 0);
      radarpoint_raw.z = dstMat.at<double>(2, 0);
      radarpoint_raw.intensity = radarcloud_msg->points[i].snr_db;
      radarpoint_raw.doppler = radarcloud_msg->points[i].v_doppler_mps;
      radarpoint_xyzi.x = dstMat.at<double>(0, 0);
      radarpoint_xyzi.y = dstMat.at<double>(1, 0);
      radarpoint_xyzi.z = dstMat.at<double>(2, 0);
      radarpoint_xyzi.intensity = radarcloud_msg->points[i].snr_db;
...
void PointProcess::avia_handler(
  ...
  if (feature_enabled) {
    ...
    for (int j = 0; j < N_SCANS; j++) {
      ...
      types[plsize].range =
          std::sqrt(pl[plsize].x * pl[plsize].x + pl[plsize].y * pl[plsize].y);
...
void PointProcess::hesai_handler(
    const sensor_msgs::PointCloud2::ConstPtr &msg) {
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();
  ...
  if (feature_enabled) {
    ...
    for (int j = 0; j < N_SCANS; j++) {
      ...
      types[linesize].range = std::sqrt(pl[linesize].x * pl[linesize].x +
...
void PointProcess::robosense_handler(
    const sensor_msgs::PointCloud2::ConstPtr &msg) {
  ...
  if (plsize == 0)
    return;
  std::sort(pl_orig.points.begin(), pl_orig.points.end(), time_list_robosense);
  pl_surf.reserve(plsize);
...
void PointProcess::give_feature(pcl::PointCloud<PointType> &pl,
...
void PointProcess::pub_func(pcl::PointCloud<PointType> &pl, ros::Publisher pub,
                            const ros::Time &ct) {
  pl.height = 1;
  pl.width = pl.size();
  ...
  output_msg.header.frame_id = "rslidar"; // note: for robosense lidar
  pub.publish(output_msg);
}

int PointProcess::plane_judge(const pcl::PointCloud<PointType> &pl,
...
bool PointProcess::edge_jump_judge(const pcl::PointCloud<PointType> &pl,
